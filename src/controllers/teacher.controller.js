...
const signup = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .cookie("TeacherAccesstoken", Accesstoken, cookieOptions)
    .cookie("TeacherRefreshtoken", Refreshtoken, cookieOptions)
    .json(new ApiResponse(200, createdTeacher, "Signup successful"));
});
...
const logout = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .clearCookie("TeacherAccesstoken", cookieOptions)
    .clearCookie("TeacherRefreshtoken", cookieOptions)
    .json(new ApiResponse(200, {}, "Logged out"));
});
...
const addTeacherDetails = asyncHandler(async (req, res) => {
  ...
  if (!Phone || !Address || !Experience) {
    throw new ApiError(400, "Phone, Address, Experience required");
  }

  const AadhaarPath = req.files?.Aadhaar?.[0]?.path;
  if (!AadhaarPath) throw new ApiError(400, "Aadhaar required");

  const Aadhaar = await uploadOnCloudinary(AadhaarPath);

  const details = await Teacherdocs.create({
    Phone,
    Address,
    Experience,
    Aadhaar: Aadhaar.url,
